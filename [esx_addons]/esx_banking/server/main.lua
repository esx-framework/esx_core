...
AddEventHandler('esx_banking:doingType', function(typeData)
	...
	if amount == nil or amount <= 0 then
		TriggerClientEvent("esx_banking:notify",source,_U('invalid_amount'),"error")
	else
		...
		elseif typeData.transfer then
			...
			TriggerClientEvent("esx_banking:notify",source,_U('not_enough_money', amount),"error")
			return
		end
		...
		if typeData.transfer then
			TriggerClientEvent("esx_banking:notify",source,_U(string.format('%s_money',key), amount,typeData.transfer.playerId),"success")
		else
			TriggerClientEvent("esx_banking:notify",source,_U(string.format('%s_money',key), amount),"success")
		end
...
ESX.RegisterServerCallback("esx_banking:getPlayerData", function(source, cb) 
	...
	local playerData = {
		...
		money = xPlayer.getAccount('money').money,
		bankMoney = xPlayer.getAccount('bank').money,
		transactionHistory = MySQL.Sync.fetchAll('SELECT * FROM banking WHERE identifier = ? and time > ? ORDER BY ID DESC', {identifier,weekAgo})
	}
...
ESX.RegisterServerCallback("esx_banking:checkPincode", function(source,cb,inputPincode)
	local xPlayer = ESX.GetPlayerFromId(source)
	local identifier = xPlayer.getIdentifier()
	local pincode = MySQL.Sync.fetchScalar('SELECT COUNT(1) AS pincode FROM users WHERE identifier = ? AND pincode = ?', {identifier,inputPincode})
	cb(pincode > 0)
end)
...
BANK = {
	...
	Transfer = function(xTarget,xPlayer,amount,key)
		if xTarget == nil or xPlayer.source == xTarget.source then 
			TriggerClientEvent("esx_banking:notify",source,_U("cant_do_it"),"error")
			return false
		end
		...
		TriggerClientEvent("esx_banking:notify",xTarget.source,_U('receive_transfer', amount, xPlayer.source),"success")
	...
	LogTransaction = function(playerId,logType,amount,bankMoney)
		...
		MySQL.insert('INSERT INTO banking (identifier, type, amount, time, balance) VALUES (?, ?, ?, ?, ?)', {identifier,logType,amount,os.time()*1000,bankMoney})
	end,
}