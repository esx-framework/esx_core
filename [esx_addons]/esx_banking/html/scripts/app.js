...
COLOR_DATA =  {
	"allTitleColor":"#ecedef",
	"yourMoneyPriceTextColor": "#45b157",
	"bankCardBackgroundColor":"linear-gradient(110deg, #d6272d 0%, #2e69d5 100%)",
	"columnsBackground":"#0a0e19",
	"buttonBackgroundColor":"#232934",
	"inputBackgroundColor":"#232934",
	"inputTextColor":"#ecedef",
	"inputPlaceholderColor":"#545457",
	"checkmarkColor":"#ecedef",
	"exitButtonHoverBackgroundColor":"#d6272d",
	"acceptButtonHoverBackgroundColor":"#2e69d5",
	"acceptExitButtonColorText":"#ecedef",
	"bankCardTextColor":"#ecedef",
	"bankCardNumberColor":"orange",
	"moneyContainerBackgroundColor":"#232934",
	"transactionContainerBackgroundColor":"#232934",
	"moneyAndTransactionallTitleColorText":"#ecedef",
	"transactionDataTextColor":"#474e57",
	"cardSubtitleTextColor":"#474e57",
	"depositColorText":"#45b157",
	"withDrawColorText":"#d6272d",
	"moreHistoryButtonHoverColor": "#3c005a",
	"moreGraphButtonHoverColor":"#08105a",
	"hrRowColor":"#474e57",
	"tableAllTextColor":"#ecedef",
	"tableBackgroundColorPaginationButton": "#2e69d5",
	"tableBackgroundColorPaginationButtonHover":"#232934",
	"tableRowHoverBackgroundColor":"#2e69d5",
	"tableScrollBarBackgroundColor":"#232934",
	"tableScrollBarBackgroundColorHover":"#2e69d5",
	"tableSelectBoxBackgroundColor":"#232934",
	"tableSearchInputBackgroundColor":"#232934",
	"disableButtonBackgroundColor":"#202020"
}

MONEY_AND_BANK_AMOUNT = [
	{
		title:"CURRENT MONEY",
		amount:"20000"
	},
	{
		title:"CURRENT BANK MONEY",
		amount:"2000"
	}
]
...
FORM_DYNAMIC_DATA = [
	...
	},
	{
		...
		buttonText:"Accept",
		inputPlaceholder: "TEST PLACEHOLDER",
		inputPlaceholder2: "TEST PLACEHOLDER2",
		title:"TRANSFER",
		description: "DESCRIPTION"
	},
	{
		...
		title:"TRANSACTIONS",
		description: "DESC TRANS",
		moreHistoryText: "Előzmények",
		moreGraphText: "Diagram"
	},
	{
		componentName:"pincodePanel",
		elementID:"#wrapper",
		title:"ENTER FUCK PINCODE",
...
$(document).ready(function(){
	...
	$(document).on('click','.pincode-numbers div:nth-child(12)',function(){
		...
		.then((response) =>{
			if(response.success){
				showNotify(response.success,"","success")
	...
	window.addEventListener('message', ({data}) => {
		...
		if(data.showMenu){
			...
			showNotify(data.data.text,"",data.data.notifyType)
		}	
	})


	function showNotify(title,text,status,speed = 300){
		new Notify ({
			title: title,
			text: text,
			position:'right top',
	...
	$(document).on('keyup','input[type="number"] , input[type="password"]',function(){
		...
		if($(this).val() != '') {
			if($(this).attr("name") == "pincode" && $(this).val().length < 4){
				if(!currentNotify){
					showNotify(language.getStoredData().showPincodeErrorTitle,language.getStoredData().showPincodeErrorMessage,"error")
					currentNotify = true
					setTimeout(function(){
						currentNotify = false
					},3000)
	...
	$(document).on('click','.accept-button',function(){
		...
		if(buttonGroup.length > 0){
			...
			if((inputValue.length == 0 && inputValue <= 0 )){
				if(!currentNotify){
					showNotify(language.getStoredData().inputErrorTitle,language.getStoredData().inputErrorMessage,"error")
					currentNotify = true
					setTimeout(function(){
						currentNotify = false
					},3000)
