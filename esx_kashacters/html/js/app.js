        ...
        if(data.characters !== null) {
            $.each(data.characters, function (index, char) {
                if (char.charid !== 0) {
                    ...
                    $('[data-charid=' + charid + ']').html('<h3 class="character-fullname">' + char.firstname + ' ' + char.lastname + '</h3><div class="character-info"><p class="character-info-work"><strong>' + `${translate.job}: ` + '</strong><span>' + char.job + '</span></p><p class="character-info-money"><strong>' + `${translate.money}: ` + '</strong><span>' + char.money + '</span></p><p class="character-info-bank"><strong>' + `${translate.bank}: ` + '</strong><span>' + char.bank + '</span></p> <p class="character-info-dateofbirth"><strong>' + `${translate.dob}: ` + '</strong><span>' + char.dateofbirth + '</span></p> <p class="character-info-gender"><strong>' + `${translate.gender}: ` + '</strong><span>'+ char.sex +'</span></p></div>').attr("data-ischar", "true");
                }
            });
        }
    };
