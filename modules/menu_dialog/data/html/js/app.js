(function () {
	...
	document.addEventListener('keyup', e => {
		...
		const { namespace, name } = ESX_MENU.focus[ESX_MENU.focus.length - 1];

		if (e.which == 27) // Escape key
			SendMessage(ESX_MENU.ResourceName, 'menu_cancel', ESX_MENU.opened[namespace][name]);
		else if (e.which == 13) // Enter key
			SendMessage(ESX_MENU.ResourceName, 'menu_submit', ESX_MENU.opened[namespace][name]);
