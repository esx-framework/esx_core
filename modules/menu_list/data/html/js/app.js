(function(){
	...
	window.ESX_MENU       = {};
	ESX_MENU.ResourceName = 'es_extended';
	ESX_MENU.opened       = {};
	ESX_MENU.focus        = [];
	ESX_MENU.data         = {};
	...
	ESX_MENU.submit = function(namespace, name, data){
		$.post('http://' + ESX_MENU.ResourceName + '/menu_list:menu_submit', JSON.stringify({
	...
	ESX_MENU.cancel = function(namespace, name){
		$.post('http://' + ESX_MENU.ResourceName + '/menu_list:menu_cancel', JSON.stringify({
			_namespace: namespace,
			_name     : name
		}));
	};
