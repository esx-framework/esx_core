	...
	TriggerEvent('esx_property:getProperties', function(properties)
		...
		for i=1, #properties, 1 do
			table.insert(elements.rows, {
				...
				cols = {
					properties[i].label,
					'{{Assigner [vente]|sell}} {{Assigner [Location]|rent}} {{GPS|gps}}'
				}
			})
		end
...
function OpenCustomersMenu()
	...
	ESX.TriggerServerCallback('esx_realestateagentjob:getCustomers', function(customers)
		...
		local elements = {
			head = {'Client', 'Propriété', 'Type', 'Actions'},
			rows = {}
		}
		...
		for i=1, #customers, 1 do
			table.insert(elements.rows, {
				...
				cols = {
					...
					(customers[i].rented and 'Location' or 'Vente'),
					'{{Révoquer contrat|revoke}} {{GPS|gps}}'
				}
			})
		end
...
Citizen.CreateThread(function()
	...
	AddTextComponentString("Agents Immobilier")
