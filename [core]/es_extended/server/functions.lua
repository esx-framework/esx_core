...
---Spawns a vehicle and optionally warps the calling player into it.
---This is the server-side replacement for the deprecated esx:spawnVehicle callback.
---Call this from your own server callback after performing contextual authorization.
...
---@param warp? boolean Whether to warp the player into the driver seat
---@param cb? fun(networkId: number) Callback when spawn completes
---@param vehicleType? string Optional vehicle type (automobile, heli, submarine, etc.)
---@return number? networkId
function ESX.SpawnVehicle(source, model, coords, heading, props, warp, cb, vehicleType)
    local promise = not cb and promise.new()

    ESX.OneSync.SpawnVehicle(model, coords, heading, props or {}, function(networkId)
        if warp and networkId then
