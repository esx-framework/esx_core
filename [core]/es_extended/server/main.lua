...
function loadESXPlayer(identifier, playerId, isNew) 
    ...
    if result.inventory and result.inventory ~= '' then
        if not Config.OxInventory then 
            ...
            for name, count in pairs(inventory) do 
                ...
                if count > 0 then
                    userData.weight += (item.weight * count)
                end
		...
		if result.loadout and result.loadout ~= '' then
            ...
            for name, weapon in pairs(loadout) do 
                ...
                if label then
                    ...
                    weapon.tintIndex = weapon.tintIndex or 0
    ...
    if not Config.OxInventory then
		...
		if isNew then
            local a = Config.StartingAccountMoney['money'] 
            if a then 
                exports.ox_inventory:AddItem(playerId, 'money', a)
            end 

            local b = Config.StartingAccountMoney['black_money']
            if b then 
                exports.ox_inventory:AddItem(playerId, 'black_money', b)
            end
		end
	end
