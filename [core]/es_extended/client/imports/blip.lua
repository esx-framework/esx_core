...
function Blip:constructor(properties)
    self.coords = type(properties.coords) == "vector3" and properties.coords or
        vector3(properties.coords.x, properties.coords.y, properties.coords.z)
    ...
    local handle = ESX.CreateBlipInternal(self.coords, self.sprite, self.colour, self.label, self.scale, self.display,
        self.shortRange, self.resource)
    self.handle = handle
    return handle
end
