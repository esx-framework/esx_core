...
function ESX.TriggerServerCallback(name, requestId, source, Invoke, cb, ...)
    if Core.ServerCallbacks[name] then
        ...
        print(('[^1ERROR^7] Server callback ^5"%s"^0 does not exist. Please Check ^5%s^7 for Errors!'):format(name,
            Invoke))
    end
end
...
function Core.SavePlayer(xPlayer, cb)
    MySQL.prepare(
        ...
        ,
        { json.encode(xPlayer.getAccounts(true)), xPlayer.job.name, xPlayer.job.grade, xPlayer.group,
            json.encode(xPlayer.getCoords()),
            json.encode(xPlayer.getInventory(true)), json.encode(xPlayer.getLoadout(true)), xPlayer.identifier },
...
function Core.SavePlayers(cb)
    ...
    if count > 0 then
        ...
        for i = 1, count do
            ...
            parameters[#parameters + 1] = { json.encode(xPlayer.getAccounts(true)), xPlayer.job.name, xPlayer.job.grade,
                xPlayer.group,
                json.encode(xPlayer.getCoords()), json.encode(xPlayer.getInventory(true)),
        ...
        MySQL.prepare(
            ...
            if results then
                if type(cb) == 'function' then
                    ...
                    print(('[^2INFO^7] Saved ^5%s^7 %s over ^5%s^7 ms'):format(count, count > 1 and 'players' or 'player'
                        , ESX.Math.Round((os.time() - time) / 1000000, 2)))
                end
            end
        end)
    end
